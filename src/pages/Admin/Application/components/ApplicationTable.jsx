...
const ApplicationTable = ({
    ...
    type,
    handleOpenModal, setPositionToDelete, setShowDeleteModal
}) => {
    return (
                ...
                <table className="w-full">
                    <thead className="bg-gray-50 border-b border-gray-200">
                        <tr>
                            ...
                            <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                {
                                    type === "Application" ? 'University' : 'Department'
                                }
                            </th>
                            ...
                            <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                {type === "Application" ? 'Status' : 'Location'}
                            ...
                            <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                {type === "Application" ? 'Date' : 'Duration'}
                        ...
                        {filteredApplications.map((application, index) => (
                            <tr key={application.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                                {type === "Application" ? (
                                    ...
                                    // For type === "Intern" render different cells, for example:
                                    <>
                                        <td className="px-6 py-4 text-sm text-gray-900">{application.position}</td>
                                        <td className="px-6 py-4 text-sm text-gray-900">{application.department}</td>
                                        <td className="px-6 py-4 text-sm text-gray-900">{application.location}</td>
                                        <td className="px-6 py-4 text-sm text-gray-900">{application.duration}</td>
                                        <td className="px-6 py-4 text-sm text-gray-900">{application.salary}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div className="flex items-center space-x-3">
                                                ...
                                                <button
                                                    onClick={() => {
                                                        setPositionToDelete(application);
